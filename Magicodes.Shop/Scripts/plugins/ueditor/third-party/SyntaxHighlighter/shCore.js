...
(function(undefined) {
    ...
    var replacementToken = /\$(?:(\d\d?|[$&`'])|{([$\w]+)})/g,
        ...
        compliantExecNpcg = nativ.exec
            .call(/()??/, "")[1] ===
            undefined, // check `exec` handling of nonparticipating capturing groups
    ...
    nativeTokens[XRegExp
        .INSIDE_CLASS] = /^(?:\\(?:[0-3][0-7]{0,2}|[4-7][0-7]?|x[\dA-Fa-f]{2}|u[\dA-Fa-f]{4}|c[A-Za-z]|[\s\S]))/;
    nativeTokens[XRegExp
            .OUTSIDE_CLASS] =
    ...
    XRegExp.iterate = function(str, regex, callback, context) {
        ...
        while (match = r2.exec(str)) { // Run the altered `exec` (required for `lastIndex` fix, etc.)
            if (regex.global)
                regex.lastIndex = r2
                    .lastIndex; // Doing this to follow expectations if `lastIndex` is checked within `callback`
    ...
    RegExp.prototype.exec = function(str) {
        ...
        if (match) {
            ...
            if (!compliantExecNpcg && match.length > 1 && indexOf(match, "") > -1) {
                ...
                nativ.replace.call((str + "").slice(match.index),
                    r2,
                    function() {
    ...
    String.prototype.replace = function(search, replacement) {
        ...
        if (Object.prototype.toString.call(replacement) === "[object Function]") {
            result = nativ.replace.call(this + "",
                search,
                function() {
                    if (captureNames) {
        ...
        try {
            while (i--) { // Run in reverse order
                ...
                if ((scope & t.scope) && (!t.trigger || t.trigger.call(context))) {
                    ...
                    match = t.pattern
                        .exec(pattern); // Running the altered `exec` here allows use of named backreferences, etc.
