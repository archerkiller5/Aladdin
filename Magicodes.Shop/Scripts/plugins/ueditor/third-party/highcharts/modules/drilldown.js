(function(e) {
    function q(b, a, c) {
        return"rgba(" +
        [
            Math.round(b[0] + (a[0] - b[0]) * c), Math.round(b[1] + (a[1] - b[1]) * c), Math
            .round(b[2] + (a[2] - b[2]) * c), b[3] + (a[3] - b[3]) * c
        ].join(",") +
        ")"
    }
    ...
    h.prototype.showDrillUpButton = function() {
        ...
        this.drillUpButton
            ...
            : this.drillUpButton = this.renderer.button(a, null, null, function() { b.drillUp() })
    ...
    h.prototype.drillUp = function() {
        ...
        if (d.type === a.type)
            d.drilldownLevel = b, d.animate = d
                .animateDrillupTo ||
                m, d.options.animation = !0, a.animateDrillupFrom && a.animateDrillupFrom(b);
        a.remove(!1);
        ...
        b ||
            g(this.points,
                function(a, b) {
                    ...
                    a.graphic.attr(e.merge(d, { start: f + b * s, end: f + (b + 1) * s }))
                        .animate(a.shapeArgs,
                            e.merge(c,
                                { step: function(a, d) { d.prop === "start" && this.attr({ fill: q(h, g, d.pos) }) } }))
                })
    };
    ...
    l.prototype.animateDrilldown = function(b) {
        var a = this.chart.drilldownLevels[this.chart.drilldownLevels.length -
            ...
            c = this.chart.options.drilldown.animation;
        b ||
        (a.x += this.xAxis.oldPos - this.xAxis.pos, g(this.points,
            function(b) { b.graphic.attr(a).animate(b.shapeArgs, c) }))
    };
            ...
            g(this.points,
                function(b) {
                    if (b.drilldown && b.dataLabel)
                        b.dataLabel.attr({ "class": "highcharts-drilldown-data-label" })
                            .css(a)
                            .on("click",
