        ...
        init: function(options, chart, firstAxis) {
            ...
            if (backgroundOption) {
                each([].concat(splat(backgroundOption)).reverse(),
                    function(config) {
                        var backgroundColor = config
                            .backgroundColor; // if defined, replace the old one (specific for gradients)
 ...
 */
    wrap(axisProto,
        "init",
        function(proceed, chart, userOptions) {
            var axis = this,
            ...
            if (!isHidden && (angular || polar)) {
                ...
                this.endAngleRad = endAngleRad = (pick(paneOptions.endAngle, paneOptions.startAngle + 360) - 90) *
                    Math.PI /
                    180;
 ...
 */
    seriesTypes.arearange = Highcharts.extendClass(seriesTypes.area,
    {
        ...
        getSegmentPath: function(segment) {
            ...
            lowSegment = HighchartsAdapter.grep(segment,
                function(point) {
            ...
            if (step) {
                ...
                options
                    .step = { left: "right", center: "center", right: "left" }[step];
 ...
 */
    ...
    seriesTypes.gauge = Highcharts.extendClass(seriesTypes
        .line,
        GaugeSeries); /* ****************************************************************************
    ...
    seriesTypes.boxplot = extendClass(seriesTypes.column,
    {
        ...
        drawPoints: function() {
            var series = this, //state = series.state,
                ...
                doQuartiles = series
                    .doQuartiles !==
                    false, // error bar inherits this series type but doesn't do quartiles
 ...
 */
    wrap(seriesTypes.spline.prototype,
        "getPointSpline",
...
// 1 means control points midway between points, 2 means 1/3 from the point, 3 is 1/4 etc;
            ...
            if (this.chart.polar) {
                ...
                if (this.connectEnds) {
                    if (!lastPoint) {
                        lastPoint = segment[segment
                            .length -
                            2]; // not the last but the second last, because the segment is already connected
                    }
                    if (!nextPoint) {
                        nextPoint = segment[1];
                    }
                ...
                if (lastPoint && nextPoint) {
                    ...
                    distanceLeftControlPoint = Math.sqrt(Math
                        .pow(leftContX - plotX, 2) +
                        Math.pow(leftContY - plotY, 2));
                    distanceRightControlPoint = Math.sqrt(Math.pow(rightContX - plotX, 2) +
