...
(function(window) {
    ...
    Slide.prototype._init = function(options) {
        ...
        for (var i = 0; i < this.slideNodeListLength; i++) {
            this.slideNodeList[i] &&
            (this.slideNodeList[i].style
                .webkitTransform = this.slideNodeList[i].style
                .transform = "translate3d(" + (this.slideWrapWidth * i) + "px,0,0)");
        }
    ...
    Slide.prototype.createPagination = function(index) {
        ...
        case "line": // 线条
            if (!this.continuous) {
                for (var i = 0; i < 2; i++) {
                    pageShowHtml += '<span class="' +
                        CLASS_SLIDE_PAGE +
                        " " +
                        CLASS_SLIDE_PAGE_LINE +
                        '" style="width:50%"></span>';
                }
            } else {
                for (var i = 0; i < this.slideNodeListLength; i++) {
