...
(function(window) {
    ...
    aui.calendar = function(element, opts, callback) {
            ...
            for (var i = startYear; i <= yearLen; i++) {
                ...
                for (var ii = startMonth; ii < 12; ii++) {
                    if (endYear == i && ii >= endMonth) {
                        ...
                        tableHtml += "<tr>";
                        tableHtml += '<th class="aui-calendar-title aui-text-primary" colspan="7">' +
                            i +
                            "年" +
                            _monthArr[ii] +
                            "</th>";
                        tableHtml += "</tr>";
                        tableHtml += '<tr class="aui-text-primary" data-year="' +
                            i +
                            '" data-month="' +
                            ii +
                            '">' +
        ...
        var _createDay = function(year, month) {
            var html = "",
                ...
                _d = 1;
            // 开始日期有传入天时计算
            if (_startDate.split("-").length > 2 &&
                year == parseFloat(_startDate.split("-")[0]) &&
                month == parseFloat(_startDate.split("-")[1])) {
                _d = parseFloat(_startDate.split("-")[2]);
            }
            var firstDay = _getFirstDay(year, month);
            // 当结束日期有传入天时计算
            if (_endDate.split("-").length > 2 &&
                year == parseFloat(_endDate.split("-")[0]) &&
                month == parseFloat(_endDate.split("-")[1])) {
                    ...
                    if (s >= firstDay && d <= monthLen) {
                        if (_d > d & beforeStartDateClick == false) {
                            html += '<td date="' +
                                ...
                                "-" +
                                _foo(d) +
                                '" class="before-day" click-status="' +
                                beforeStartDateClick +
                                '">';
                        } else {
                            if (col == 0 || col == 6) {
                                html += '<td date="' +
    ...
    aui.sliderUp = function(element, callback) {
                    ...
                    for (i; i < m; i++) {
                        ...
                        if (i == self.initIndex + 1) {
                            ...
                            (lis[i].style
                                ...
                                ((i - self.initIndex) * self.winH + self.offsetY) +
                                "px, 0)");
                        }
                        if (i == self.initIndex) {
                            ...
                            (lis[i].style
                                .webkitTransform = "translate3d(0, " +
                                ((i - self.initIndex) * self.winH + self.offsetY * 0.3) +
                                "px, 0) scale(" +
                                (1 - (self.offsetY / (self.winH * 3.5))) +
                                ")");
                        }
                        if (i == self.initIndex - 1) {
                            ...
                            (lis[i].style
                                .webkitTransform = "translate3d(0, " +
                                ((i - self.initIndex) * self.winH + self.offsetY + 50) +
                                "px, 0)");
                        }
                    }
                } else { //up
        ...
        var _goIndex = function(n) {
            ...
            lis[currIndex - 1] &&
            (lis[currIndex - 1].style.webkitTransform = "translate3d(0, " + -this.winH + "px, 0)"
            );
