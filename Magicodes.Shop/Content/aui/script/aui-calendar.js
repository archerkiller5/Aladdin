...
(function(window) {
    ...
    calendar.prototype._createDay = function(year, month) {
        ...
        var html = "",
            ...
            _d = 1;
        // 开始日期有传入天时计算
        if (this._startDate.split("-").length > 2 &&
            year == parseFloat(this._startDate.split("-")[0]) &&
            month == parseFloat(this._startDate.split("-")[1])) {
            _d = parseFloat(this._startDate.split("-")[2]);
        }
        var firstDay = this._getFirstDay(year, month);
        // 当结束日期有传入天时计算
        if (this._endDate.split("-").length > 2 &&
            year == parseFloat(this._endDate.split("-")[0]) &&
            month == parseFloat(this._endDate.split("-")[1])) {
                ...
                if (s >= firstDay && d <= monthLen) {
                    if (_d > d & this.options.beforeStartDateClick == false) {
                        html += '<td date="' +
                            ...
                            this.options.beforeStartDateClick +
                            '">';
                    } else {
                        if (col == 0 || col == 6) {
                            html += '<td date="' +
                    ...
                    if (this.options.lunarShow) {
                        html += "<p>" +
                            this._getLunarDay("" + year + "-" + this._foo(month) + "-" + this._foo(d) + "") +
                            "</p>";
                    }
                    d++;
                } else {
