...
namespace Magicodes.Shop.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                var user = await UserManager.FindAsync(model.UserName.Trim(), model.Password);
                if (user != null)
                {
                    try
                    {
                        ...
                        //登陆日志
                        LoginSuccess(model.UserName, user.Id,user.TenantId);
                    }
                    catch (Exception ex)
                    {
                        ModelState.AddModelError("", ex.Message);
                        //登陆日志,失败
                        LoginFail(model.UserName, model.Password);
                        return View(model);
                    }
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.Error ? "发生异常，请联系管理员。"
                : "";
